...
구현
------------------------------------
-  `웹페이지 to Web API`_ 와 유사하게 구조를 가진다.
-  다른 점은 개인정보보호를 위해 암호화된 URL규칙으로부터 웹페이지에 로그인이 가능하도록 협의한다.
-  로그인 후 약속된 페이지를 PDF로 실시간 편집/변환한다.
...
주의점
------------------------------------
-  ID/비밀번호를 그대로 노출하는 것보다 익명성을 제공하거나, OAuth등 검증된 접근허가 방식이 필요하다.
-  웹페이지 그대로를 ``PDF`` 로 변환하는 편이 좋다. 숨겨진 화면이나 이벤트가 필요하다면 지나친 커스터마이징이 가미된다.


기타
------------------------------------
M2는 고객에 의해 접근이 허가된 페이지에만 접근할 수 있다.