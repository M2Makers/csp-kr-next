...
.. _pattern-image-legacy-url-gateway:
...
레거시 URL로부터 원본 이미지를 찾을 수 있는 방식(URL해석 또는 이미지정보 API 연동)만 제공되면 단일 시스템으로 운영이 가능하다.


구현
------------------------------------
-  ``M2`` 를 이미지 게이트웨이의 역할로 백엔드 가장 앞단에 배치한다.
-  ``M2`` 에 레거시 이미지 URL규칙을 이식한다.
-  ``M2`` 연동을 원하는 백엔드를 순차적으로 연결한다. 
   예제의 ``Service Call Chain`` 은 다음과 같이 동작한다.
   
   1. 호출된 URL 주소의 의미를 해석한다. M2용 Hooking 함수를 작성한다.
   2. 이미지 정보서버가 존재한다면 API를 연동하여 이미지 위치와 처리방식을 결정한다.
   3. 이미지를 다운로드 받는다. 이미지 서버는 HTTP/S호출만 가능하다면 어디든 무관한다.
...
-  최종 이미지 뿐만 아니라 모든 연결지점에 캐싱 적용이 사용할 수 있다. 
   예를 들어 API Server 가용량이 부족할 경우 증설하지 않고 캐싱으로 가용량 조절이 가능하다.
-  2-Tier 또는 Sibling 분산을 통해 병렬처리가 용이하다.


주의점
------------------------------------
-  기본적인 서비스 시나리오가 복잡하기 때문에 외부 의존성에 대해 캐싱정책 및 Default 동작등 Fallback을 정해둔다. 
-  장애시 블랙박스가 되지 않도록 구축/검수시 예외상황에 대한 케이스를 정리해둔다.


기타
------------------------------------
레거시 URL 및 비지니스 로직은 단순한 패턴 맵핑의 경우도 있지만, 개발로직이 들어가는 경우도 많다.
이런 상황을 대비해 M2는 ``JavaScript`` 로 비지니스 로직을 구현할 수 있도록 지원한다.